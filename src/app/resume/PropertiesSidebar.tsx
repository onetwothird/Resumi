...
interface Props {
  ...
  // When true (e.g. arriving via ?scan=ats from the hiring roadmap), runs the ATS scan once on load
  autoRunAts?: boolean;
}
...
export default function PropertiesSidebar({ data, onChange, pushToast, autoRunAts }: Props) {
  ...
  // Deep-linked from the hiring roadmap's "Run ATS Scan" action — fire once on arrival
  useEffect(() => {
    if (!autoRunAts || hasAutoRun.current) return;
    hasAutoRun.current = true;
    if (data.summary?.trim()) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      analyzeATS();
    } else {
      pushToast("Add a professional summary, then hit Scan Resume.", "error");
    }
