...
import { ResumeData, DEFAULT_THEME, getFontStack, ResumeFontSize } from "@/types";
...
const ResumePreview = forwardRef<HTMLDivElement, Props>(({ data }, ref) => {
  ...
  if (theme.layout === "modern") {
    return (
      ...
      >
        ...
        >
          <h1 className={`${sizes.name} font-bold tracking-wide mb-1`}>
            {data.firstName} {data.lastName}
          </h1>
          <p className={`${sizes.title} opacity-90`}>{data.jobTitle}</p>
          {contactLine.length > 0 && (
            <div className={`${sizes.meta} opacity-80 flex flex-wrap gap-x-4 gap-y-1 mt-3`}>
              {contactLine.map((c, i) => (
                <span key={i}>{c}</span>
              ))}
            </div>
        ...
        <div className="px-12 py-8">
          ...
          <p className={`${sizes.body} leading-relaxed text-gray-700 whitespace-pre-wrap`}>
            {data.summary}
          </p>
        </div>
      </div>
  ...
  if (theme.layout === "minimal") {
    return (
      ...
      >
        <div className="mb-10">
          <h1 className={`${sizes.name} font-semibold tracking-tight mb-1 text-gray-900`}>
            {data.firstName} {data.lastName}
          </h1>
          <p className={`${sizes.title} font-medium mb-2`} style={{ color: accent }}>
            {data.jobTitle}
          </p>
          {contactLine.length > 0 && (
            <div className={`${sizes.meta} text-gray-500 flex flex-wrap gap-x-3 gap-y-1`}>
              {contactLine.map((c, i) => (
                <span key={i}>
        ...
        <div>
          ...
          <p className={`${sizes.body} leading-loose text-gray-700 whitespace-pre-wrap`}>
            {data.summary}
          </p>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="text-center border-b pb-6 mb-6" style={{ borderColor: accent }}>
        <h1 className={`${sizes.name} font-bold uppercase tracking-wider mb-1`}>
          {data.firstName} {data.lastName}
        </h1>
        <p className={`${sizes.title} text-gray-600 mb-3`}>{data.jobTitle}</p>
        {contactLine.length > 0 && (
          <div className={`${sizes.meta} text-gray-500 flex justify-center gap-4`}>
            {contactLine.map((c, i) => (
