...
export default function JobsClient({ isLoggedIn }: Props) {
  ...
  const fetchJobs = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      setError("Couldn't load job listings. Try again shortly.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    fetchJobs();
    const interval = setInterval(() => fetchJobs({ silent: true }), 30_000);
  ...
  const filteredJobs = useMemo(() => {
    ...
    let result = jobs.filter((job) => {
      const matchesQuery = !q || job.title.toLowerCase().includes(q) || job.company.toLowerCase().includes(q) || skillsOf(job).some((s) => s.toLowerCase().includes(q));
      const matchesLocation = !locQ || (job.remote && "remote".includes(locQ)) || (job.location ?? "").toLowerCase().includes(locQ);
  ...
  const content = (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out max-w-6xl mx-auto">
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight mb-2">Job Matches</h1>
          <p className="text-slate-500 font-medium">Browse roles employers have posted, updated in real time.</p>
        </div>
      </div>
      ...
      <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200/80 flex flex-col md:flex-row items-center gap-2 mb-10 transition-shadow focus-within:shadow-md focus-within:border-indigo-300">
        <div className="relative flex-1 w-full flex items-center">
          ...
          <input type="text" placeholder="Search by job title, skill, or company..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-transparent border-none outline-none text-sm font-medium text-slate-900 placeholder:text-slate-400" />
        </div>
        ...
        <div className="relative flex-1 w-full items-center hidden md:flex">
          ...
          <input type="text" placeholder="Location or 'Remote'" value={locationQuery} onChange={(e) => setLocationQuery(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-transparent border-none outline-none text-sm font-medium text-slate-900 placeholder:text-slate-400" />
        </div>
        <button onClick={() => fetchJobs()} className="w-full md:w-auto bg-slate-900 hover:bg-slate-800 text-white font-semibold py-3 px-8 rounded-xl transition-all shadow-sm active:scale-95 whitespace-nowrap">Search</button>
      </div>

      <button onClick={() => setFiltersOpen((v) => !v)} className="w-full flex lg:hidden items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-slate-200/80 mb-4 font-semibold text-slate-900">
        <span className="flex items-center gap-2">
          <SlidersHorizontal className="w-4 h-4" /> Filters
          {(selectedTypes.size + selectedModels.size) > 0 && (
            <span className="bg-indigo-600 text-white text-[11px] font-bold w-5 h-5 rounded-full flex items-center justify-center">{selectedTypes.size + selectedModels.size}</span>
          )}
        </span>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <aside className={`w-full lg:w-56 shrink-0 ${filtersOpen ? "block" : "hidden"} lg:block`}>
          <div className="lg:sticky lg:top-24 bg-white lg:bg-transparent p-5 lg:p-0 rounded-2xl lg:rounded-none border lg:border-none border-slate-200/80 shadow-sm lg:shadow-none mb-4 lg:mb-0">
            <div className="hidden lg:flex items-center gap-2 font-bold text-slate-900 mb-6"><SlidersHorizontal className="w-4 h-4" /> Filters</div>
                  ...
                  {JOB_TYPES.map((type) => (
                    <label key={type} className="flex items-center gap-3 cursor-pointer group">
                      <div className="relative flex items-center justify-center">
                        <input type="checkbox" checked={selectedTypes.has(type)} onChange={() => toggleType(type)} className="peer appearance-none w-4 h-4 rounded border border-slate-300 checked:bg-indigo-600 checked:border-indigo-600 transition-colors cursor-pointer" />
                ...
                <div className="space-y-3">
                  {WORK_MODELS.map((model) => (
                    <label key={model} className="flex items-center gap-3 cursor-pointer group">
                      <div className="relative flex items-center justify-center">
                        <input type="checkbox" checked={selectedModels.has(model)} onChange={() => toggleModel(model)} className="peer appearance-none w-4 h-4 rounded border border-slate-300 checked:bg-indigo-600 checked:border-indigo-600 transition-colors cursor-pointer" />
                        <CheckIcon className="absolute w-3 h-3 text-white opacity-0 peer-checked:opacity-100 pointer-events-none" />
                      </div>
        ...
        <div className="flex-1 space-y-5">
          <div className="flex justify-between items-center mb-4">
            <span className="text-sm font-medium text-slate-500">{isLoading ? "Loading jobs…" : `Showing ${filteredJobs.length} job${filteredJobs.length === 1 ? "" : "s"}`}</span>
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortOption)} className="bg-transparent text-sm font-semibold text-slate-700 outline-none cursor-pointer hover:text-indigo-600 transition-colors">
          ...
          {!isLoading && error && (
            <div className="bg-white border border-red-100 rounded-2xl p-10 text-center">
              ...
              <button onClick={() => fetchJobs()} className="inline-flex items-center gap-2 text-sm font-semibold bg-slate-900 text-white px-5 py-2.5 rounded-xl hover:bg-slate-800 transition-colors">Try again</button>
            </div>
          )}
          ...
          {!isLoading && !error && filteredJobs.map((job) => {
            ...
            return (
              <div key={job.id} className="group bg-white p-6 md:p-8 rounded-3xl border border-slate-200/60 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-300 flex flex-col md:flex-row gap-6 relative">
                {job.posterImageUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={job.posterImageUrl} alt="" className="w-14 h-14 rounded-2xl object-cover shrink-0 border border-slate-100" />
                ) : (
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-bold shrink-0 ${logoColorFor(job.company)}`}>{job.company.charAt(0).toUpperCase()}</div>
                )}

                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-2 gap-3">
                    <div className="min-w-0">
                      <h3 className="text-lg font-bold text-slate-900 group-hover:text-indigo-600 transition-colors cursor-pointer mb-1 wrap-break-word">{job.title}</h3>
                      <p className="text-sm font-medium text-slate-500 wrap-break-word">{job.company}</p>
                    </div>
                    <button className="text-slate-300 hover:text-indigo-500 transition-colors p-1 shrink-0"><Bookmark className="w-5 h-5" /></button>
                  </div>

                  <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-slate-500 mb-5 mt-4">
                    <span className="flex items-center gap-1.5"><MapPin className="w-4 h-4 text-slate-400" /> {job.remote ? "Remote" : job.location || "Location not specified"}</span>
                    <span className="flex items-center gap-1.5"><Briefcase className="w-4 h-4 text-slate-400" /> {job.employmentType}</span>
                    {salary && <span className="flex items-center gap-1.5"><Wallet className="w-4 h-4 text-slate-400" /> {salary}</span>}
                    <span className="flex items-center gap-1.5" suppressHydrationWarning><Clock className="w-4 h-4 text-slate-400" /> {timeAgo(job.createdAt)}</span>
                  </div>

                  {skills.length > 0 && (
                    <div className="flex flex-wrap items-center gap-2">
                      {skills.map((tag) => (
                        <span key={tag} className="bg-slate-100 text-slate-600 px-3 py-1 rounded-lg text-xs font-semibold max-w-full truncate">{tag}</span>
                      ))}
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center justify-end shrink-0 pt-4 md:pt-0 mt-4 md:mt-0 border-t md:border-t-0 border-slate-100">
                  <Link href={`/jobs/${job.id}`} className="w-full md:w-auto flex justify-center items-center gap-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-600 hover:text-white font-bold py-2.5 px-6 rounded-xl transition-colors">
          ...
          {isLoading && jobs.length > 0 && (
            <div className="flex justify-center py-4 text-slate-400 text-xs gap-2 items-center"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Refreshing…</div>
          )}
        </div>
      </div>
    </div>
      ...
      <header className="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 lg:px-6 sticky top-0 z-20 shadow-xs">
        <div className="flex items-center gap-4 lg:gap-8">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-500">
            ...
            <Link href="/jobs" className="text-gray-900 font-semibold transition-colors">Jobs</Link>
